...
public class AdAlgorithms {
    ...
    public static Integer getAdIdToDisplay(Integer areaId){

        List<CampaignInfo> campaignInfoList = campaignInfoDAO.getAdsForArea(areaId);
        Collections.sort(campaignInfoList);

        int size = campaignInfoList.size();
        int no = getRandomNoBetween(0, size-1);
        return campaignInfoList.get(no).getAdId();
    }
